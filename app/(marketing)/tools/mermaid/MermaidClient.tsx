...
export default function MermaidClient() {
  ...
  useEffect(() => {
    ...
    const loadMermaid = async () => {
      try {
        ...
        if (!mermaid) {
          throw new Error('Mermaid not loaded in browser environment')
        }
        ...
        setRenderError(err instanceof Error ? err.message : 'Mermaid loading error')
      }
    }
  ...
  useEffect(() => {
    ...
    const render = async () => {
      try {
        ...
        setSvg('')
        setRenderError(err instanceof Error ? err.message : 'Mermaid rendering failed')
      }
    }
  ...
  const downloadSvg = () => {
    ...
    toast.success('Start Download SVG')
  }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-[calc(100vh-140px)] flex flex-col">
      <div className="flex items-center justify-between border-b pb-4 shrink-0">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={downloadSvg} disabled={!svg}> <Download className="h-4 w-4 lg:mr-2" /><span className="hidden lg:inline">Download SVG</span></Button>
        </div>
      </div>
      ...
      {renderError && (
        <Alert variant="destructive">
          ...
          <AlertTitle>{isMermaidReady ? 'Render Error' : 'Load Error'}</AlertTitle>
      ...
      <div className="flex-1 min-h-0 grid grid-rows-2 lg:grid-cols-2 lg:grid-rows-1 gap-4 lg:gap-6">
        <Card className="flex flex-col min-h-0 border-0 shadow-lg ring-1 ring-border">
          ...
          <CardContent className="p-0 flex-1 min-h-0"><Textarea value={source} onChange={(e) => setSource(e.target.value)} className="h-full min-h-full resize-none rounded-none border-0 font-mono text-xs" placeholder="Enter Mermaid diagram syntax" /></CardContent>
        </Card>
