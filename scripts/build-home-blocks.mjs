...
function main() {
  ...
  // Block 2: 🆚 Most Compared（被找替代最多的付费工具）  
  {
    ...
    blocks.push({
      ...
      resources: comparisonItems.slice(0, 8).map(c => ({
        id: (c.paidTool || 'tool') + '-vs-' + (c.freeAlternativeName || 'alt'),
        name: `${c.paidTool || 'Tool'} vs ${c.freeAlternativeName || 'Alternative'}`,
        description: `Compare ${c.paidTool || 'tool'} with free alternatives`,
        url: `/directory/compare/${encodeURIComponent(c.paidTool || 'tool')}/${encodeURIComponent(c.freeAlternativeName || 'alt')}`,
        category: 'comparison',
