...
//
// Models (all via @imgly/background-removal):
//   - "isnet"        : ISNet-UAN, ~5MB, fast, general purpose  (default)
//   - "isnet_fp16"   : ISNet-UAN fp16 weights, ~3MB, slightly better precision
//   - "isnet_quint8" : ISNet-UAN quantized, ~2MB, fastest, lower quality
...
export type BGRemovalModel = "standard" | "enhanced";

/** Internal model name passed to @imgly/background-removal */
function toInternalModel(model: BGRemovalModel): "isnet" | "isnet_fp16" {
  return model === "enhanced" ? "isnet_fp16" : "isnet";
}

// ─── Preload ───────────────────────────────────────────────────────────

let _preloadedModel: BGRemovalModel | null = null;

/** Preload the ISNet model so first removal is faster. Safe to call multiple times. */
export async function preloadModel(
  model: BGRemovalModel = "standard",
  onProgress?: (msg: string, pct: number) => void
): Promise<void> {
  if (_preloadedModel === model) return;
  onProgress?.(`Loading ${model === "enhanced" ? "enhanced" : "AI"} model…`, 10);
  const ml = await getMLModule();
  if (!ml) throw new Error("ML module not loaded");
  await ml.preload({
    model: toInternalModel(model),
    progress: (key, current, total) => {
      if (key.includes("downloading") || key.includes("computing")) {
        onProgress?.("Loading model…", Math.round((current / total) * 90));
      }
    },
  });
...
/** Preload alias for enhanced model */
export async function preloadEnhanced(
  onProgress?: (msg: string, pct: number) => void
): Promise<void> {
  return preloadModel("enhanced", onProgress);
}

// ─── ML-based background removal (primary) ─────────────────────────────

interface RemoveBackgroundMLOptions {
...
/**
 * Remove background using an ISNet ML model.
 * Returns ImageData with alpha channel (transparent where bg was removed).
 *
 * @param imageData - Source image data
 * @param options.model - "standard" (isnet, fast) or "enhanced" (isnet_fp16, slightly better edges)
 */
export async function removeBackgroundML(
  imageData: ImageData,
  options: RemoveBackgroundMLOptions = {}
): Promise<ImageData> {
  const { model = "standard", onProgress } = options;

  onProgress?.(5);

  // Ensure correct model is loaded
  if (_preloadedModel !== model) {
    await preloadModel(model, (msg, pct) => onProgress?.(Math.round(pct * 0.3)));
  }
