  ...
  const handleBgRemove = useCallback(async () => {
    ...
    try {
      const canvas = await (async () => {
        // Convert source image to ImageData for removeBackgroundML
        const srcCanvas = document.createElement("canvas");
        const srcEl = sourceImage as HTMLImageElement & HTMLCanvasElement;
        srcCanvas.width = srcEl.naturalWidth || srcEl.width || 0;
        srcCanvas.height = srcEl.naturalHeight || srcEl.height || 0;
        srcCanvas.getContext("2d")!.drawImage(srcEl as HTMLImageElement, 0, 0);
        const srcData = srcCanvas.getContext("2d")!.getImageData(0, 0, srcCanvas.width, srcCanvas.height);
        const resultData = await removeBackgroundML(srcData, {
          onProgress: (p) => { setProgress(p); setProgressLabel(p < 90 ? "AI processing…" : "Finalizing…"); },
        });
        const outCanvas = document.createElement("canvas");
        outCanvas.width = resultData.width; outCanvas.height = resultData.height;
